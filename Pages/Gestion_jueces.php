...
<?php $PageTitle="Gestion de jueces";
...
<?php
$txtNombreJuez = (isset($_POST['txtNombreJuez']))?$_POST['txtNombreJuez']:"";
$txtApellidoJuez = (isset($_POST['txtApellidoJuez']))?$_POST['txtApellidoJuez']:"";
  ...
  // aplicamos un try catch que señala un bloque de instrucciones a intentar ( try ), y especifica una respuesta si se produce una excepción ( catch ).
         try {
           // intentamos preparar un consulta para traer los jueces, gracias a la variable conexion traida del conector.php encargada de realizar la conexion con la base de datos
            $sentenciaSQL=$conexion->prepare("INSERT INTO juez (JUEZ_NOMBRE,JUEZ_APELLIDO,CARGO_JUEZ_ID) VALUES (:JUEZ_NOMBRE,:JUEZ_APELLIDO,:CARGO_JUEZ_ID); ");
            //una vez preparada la consulta procedemos a asignar variables a la consulta
            $sentenciaSQL->bindParam(':JUEZ_NOMBRE',$txtNombreJuez);
            $sentenciaSQL->bindParam(':JUEZ_APELLIDO',$txtApellidoJuez);
            $sentenciaSQL->bindParam(':CARGO_JUEZ_ID',$selectCargo);
          ...
          toastr.success("¡Juez agregado correctamente!");
          setTimeout(() => {
        ...
        // si tenemos algun problema al realziar la consulta hacemos un echo trayendo el toastr de error
          ...
          toastr.error("Ocurrio un error al agregar un juez");
          setTimeout(() => {
          location.href = "Gestion_jueces.php";
          }, 3000);
          </script>';
...
<?php
try {
    $sentenciaSQL=$conexion->prepare("SELECT JUEZ_ID, JUEZ_NOMBRE, JUEZ_APELLIDO, cargo_juez.CARGO_JUEZ_NOMBRE FROM `juez`
    INNER JOIN cargo_juez on juez.CARGO_JUEZ_ID = cargo_juez.CARGO_JUEZ_ID
    WHERE JUEZ_FECHA_BAJA IS NULL;");
    $sentenciaSQL->execute();
...
try {
    $sentenciaSQL=$conexion->prepare("SELECT * FROM `cargo_juez` WHERE CARGO_FECHA_BAJA IS NULL");
    $sentenciaSQL->execute();
  ...
  <script>
		...
		toastr.options =
		{
		  ...
		  "progressBar" : true
			  ...
			  toastr.error("Error al editar juez!");
			  setTimeout(() => {
        location.href = "Gestion_jueces.php";
        }, 1500);
   ...
   <?php if ($_GET["msg"]=="error") { ?>
		toastr.options =
		{
		  ...
		  "progressBar" : true
			  ...
			  toastr.error("Error al borrar juez!");
			  setTimeout(() => {
        location.href = "Gestion_jueces.php";
        }, 1500);
...
<div class="row">
                      <h4 class="fw-bold py-3 mb-4"><span class="text-muted fw-light">Gestion de personal /</span> Gestion de jueces</h4>
                        <div class="col-12 col-lg-4 order-2 order-md-3 order-lg-2 mb-2">
                            <div class="card">
                                <div class="d-flex align-items-end row">      
                                    <div class="col-md-11">
                                            <div class="card-body">
                                                <h5 class="card-title text-primary">Gestion de jueces</h5>
                                                <h6 class="card-subtitle text-muted">Permite agregar/modificar/eliminar un juez.</h6>
                                            </div>
                                        <div class="card-body">
                                            <form method="POST" enctype="multipart/form-data">

                                                <div class="mb-3">
                                                <label for="exampleFormControlSelect1" class="form-label">Ingrese nombre del juez:</label>
                                                    <input type="text" required ="true" class="form-control"name="txtNombreJuez" id="txtNombreJuez" autocomplete="off" placeholder="Ingrese nombre del juez"onkeydown="preventNumberInput(event)" onkeyup="preventNumberInput(event)">                                       
                                                </div>

                                                <div class="mb-3">
                                                <label for="exampleFormControlSelect1" class="form-label">Ingrese apellido del juez:</label>
                                                    <input type="text" required ="true" class="form-control" name="txtApellidoJuez" id="txtApellidoJuez" autocomplete="off" placeholder="Ingrese apellido del juez"onkeydown="preventNumberInput(event)" onkeyup="preventNumberInput(event)">                                       
                                                </div>

                                                <div class="mb-3">
                                                  <label for="exampleFormControlSelect1" class="form-label">Ingrese cargo del juez:</label>
                                                    <select class="form-select" name="selectCargo" id="selectCargo" required>
                                                    <?php foreach($listaCargos as $cargos) { ?>
                                                        <option value="none" selected disabled hidden>Seleccione un cargo</option>
                                                        <option value='<?php echo $cargos["CARGO_JUEZ_ID"] ?> '><?=$cargos["CARGO_JUEZ_NOMBRE"]?></option> 
                                                    <?php }?>
                                                    </select>                                       
                                                </div>
                        ...
                        <div class="col-12 col-lg-7 order-2 order-md-3 order-lg-2 mb-2">
                            <div class="card">
                                <div class="d-flex align-items-end row">      
                                    <div class="col-md-11">
                                            <div class="card-body">
                                                <h5 class="card-title text-primary">Tabla de jueces</h5>
                                                <h6 class="card-subtitle text-muted">Muestra informacion de todos los jueces</h6>
                                            </div>
                                        <div class="card-body">
                                           ...
                                           <thead class="thead-inverse">
                                            ...
                                            <tbody>
                                                ...
                                                <tr>
                                                    ...
                                                    <td scope="row" hidden><?php echo $jueces['JUEZ_ID'];?></td>
                                                    <td><?php echo $jueces['JUEZ_NOMBRE'];?></td>   
                                                    <td><?php echo $jueces['JUEZ_APELLIDO'];?></td> 
                                                    <td><?php echo $jueces['CARGO_JUEZ_NOMBRE'];?></td> 

                                                    <td>
                                                    <!-- Colocamos un link para dirigir a una pantalla modal con la id del juez -->

                                                        <div class="row">
                                                            <div class="col-6">
                                                            <a href="#" data-bs-target="#edit_<?php echo $jueces['JUEZ_ID']; ?>" data-bs-toggle="modal" class="btn btn-warning btn-sm"><span class="glyphicon glyphicon-edit"></span> Editar</a>
                                                            </div>
                                                           <div class="col-6">
                                                           <a href="#" data-bs-target="#delete_<?php echo $jueces['JUEZ_ID']; ?>" data-bs-toggle="modal" class="btn btn-danger btn-sm"><span class="glyphicon glyphicon-trash"></span> Borrar</a>
                                                           </div>
                                                    ...
                                                    <?php include('Jueces/Modal_juez.php'); ?>
                                                </tr>
                                                <?php }?>
                                            </tbody>
                                         </table>
